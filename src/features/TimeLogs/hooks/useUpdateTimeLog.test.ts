...
import { useUpdateTimeLog } from './useUpdateTimeLog'

function useUpdateTimeLogProbe() {
  const context = useWorkspaceContext()
  const mutation = useUpdateTimeLog()
...
describe('useUpdateTimeLog', () => {
  ...
  it('updates a time log and invalidates related queries', async () => {
    setToken('test-token')
    ...
    const { result } = renderHook(() => useUpdateTimeLogProbe(), {
