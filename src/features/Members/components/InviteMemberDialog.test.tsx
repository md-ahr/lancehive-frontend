...
describe('InviteMemberDialog', () => {
  ...
  it('rejects invalid email before submit', async () => {
    ...
    await userEvent.type(screen.getByLabelText('Name'), 'New Member')
  ...
  it('maps 422 validation errors to form fields', async () => {
    ...
    await userEvent.type(screen.getByLabelText('Email'), 'duplicate@studio.test')
    await userEvent.click(screen.getByRole('button', { name: /send invitation/i }))
  ...
  it('closes dialog and shows success toast on invite', async () => {
    ...
    await userEvent.type(screen.getByLabelText('Name'), 'New Member')
    await userEvent.type(screen.getByLabelText('Email'), 'new@studio.test')
    await userEvent.click(screen.getByRole('button', { name: /send invitation/i }))
  ...
  it('disables submit while pending', async () => {
    ...
    server.use(
      http.post(`${API_BASE_URL}/members`, async () => {
    ...
    await userEvent.type(screen.getByLabelText('Name'), 'Pending Member')
    await userEvent.type(screen.getByLabelText('Email'), 'pending@studio.test')
    await userEvent.click(screen.getByRole('button', { name: /send invitation/i }))
  ...
  it('shows toast on 403 forbidden', async () => {
    ...
    await userEvent.type(screen.getByLabelText('Name'), 'Blocked Member')
    await userEvent.type(screen.getByLabelText('Email'), 'blocked@studio.test')
    await userEvent.click(screen.getByRole('button', { name: /send invitation/i }))
